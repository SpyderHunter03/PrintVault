#!/usr/bin/env bash
# ------------------------------------------------------------------
# PrintVault installer / updater — runs INSIDE a Debian/Ubuntu
# machine or LXC (as root).
#
# Fresh install or update, straight from GitHub:
#   bash -c "$(curl -fsSL https://raw.githubusercontent.com/SpyderHunter03/PrintVault/main/install/printvault-install.sh)"
#
# Or with a local source tarball instead of GitHub:
#   bash printvault-install.sh /path/to/printvault.tar.gz
#
...
if [ -n "$TARBALL" ]; then
  ...
  # copy source over the app dir, leaving data/ and node_modules/ alone
...
msg "Creating service user and data directory…"
id -u printvault >/dev/null 2>&1 || useradd -r -s /usr/sbin/nologin -d "$APP_DIR" printvault
mkdir -p "$DATA_DIR"
chown -R printvault:printvault "$APP_DIR"
...
# Optional: uncomment and add your free Thingiverse app token to enable
# full Thingiverse imports (files + images):
...
if systemctl is-active -q printvault; then
  ...
  if [ "$UPDATE" -eq 1 ]; then
    ok "PrintVault updated →  http://${IP:-<this-machine>}:${PORT}"
  else
